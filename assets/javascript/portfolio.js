...
(function() {
	...
	$(document).ready(function() {
		if (window.mobileAndTabletCheck()) {
			...
			$("#background").replaceWith('<div id="background" style="background-image: url(assets/others/mobile-background.jpg);"></div>');
			$("#audio").remove();
	...
	(function() {
		$.getJSON('https://api.ipdata.co/?api-key=ad7a1e62b6f8c99a92da5c98c776ab1353b02f0ba1f8c8306652b903', function(data) {
			writeLine(["<i style='color: #F62459'>it's unbelievable how much the world has changed in one year .while we wait to gather around the peace...</i>"], 80, function() {
				if (app.skippedIntro) return;
				clearCursor();
				var usernames = ["user", "dude"];
			writeLine(["> " + ((data.ip) ? data.ip : usernames[Math.floor(Math.random() * usernames.length)])], 80, function() {
					if (app.skippedIntro) return;
					clearCursor();
					writeLine(["> " + ((data.country_name) ? data.country_name : 'Your country')], 80, 500, function() {
